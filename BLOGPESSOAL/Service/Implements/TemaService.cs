...
namespace BLOGPESSOAL.Service.Implements
{
    public class TemaService : ITemaService
    {
        ...
        public async Task<Tema?> GetById(long id)
        {
            try
            {
                var tema = await _context.Temas.Include(t => t.Postagem).FirstAsync(i => i.Id == id);
                return tema;
            }
            catch
            {
        ...
        {
            var tema = await _context.Temas.Include(t => t.Postagem).Where(t => t.Descricao.Contains(descricao)).ToListAsync();
            return tema;
        }
